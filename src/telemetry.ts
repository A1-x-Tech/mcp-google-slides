 ...
 * environment versions ever leave the machine. The OAuth credentials,
 * presentation content, tool arguments, prompts and the names or values of
 * environment variables are never read, serialized or sent. Sends are capped
 * at 2 s and swallow every error — telemetry must never affect the server's
 * operation. Opt out for every server of this line at once: ASKADS_TELEMETRY=0
 ...
 * comparable across this line of servers. `unconfigured_start` is its live
 * counterpart — the server survives missing credentials and completes the
 * handshake, so this is the first event that can carry the client's name for
 * an install with no credentials yet.
 */
